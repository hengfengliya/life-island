...
function validateBottle(bottle) {
  ...
  if (bottle.message.trim().length === 0 || bottle.message.length > 1e3) {
    return false;
  }
  return true;
}
__name(validateBottle, "validateBottle");
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      if (path === "/api/bottles" && method === "GET") {
        return await handleGetBottles(request, env2);
      }
      if (path === "/api/bottles" && method === "POST") {
        return await handleCreateBottle(request, env2);
      }
      if (path.startsWith("/api/bottles/") && method === "GET") {
        const bottleId = path.split("/")[3];
        return await handleGetBottle(bottleId, env2);
      }
      if (path === "/api/search" && method === "GET") {
        return await handleSearchBottles(request, env2);
      }
...
async function handleGetBottles(request, env2) {
  const url = new URL(request.url);
  ...
  try {
    ...
    const keys = await env2.BOTTLES_KV.list(listOptions);
    const bottles = [];
    for (const key of keys.keys) {
      const bottleData = await env2.BOTTLES_KV.get(key.name);
      if (bottleData) {
        try {
    ...
    console.error("Error fetching bottles:", error3);
    return jsonResponse({ error: "Failed to fetch bottles" }, 500);
  }
}
__name(handleGetBottles, "handleGetBottles");
async function handleCreateBottle(request, env2) {
  try {
    ...
    await env2.BOTTLES_KV.put(key, JSON.stringify(bottle));
    const timeKey = `time:${now}:${bottleId}`;
    await env2.BOTTLES_KV.put(timeKey, bottleId);
    return jsonResponse({
      success: true,
      bottle,
      message: "\u601D\u5FF5\u5DF2\u7ECF\u4EA4\u7ED9\u5927\u6D77 \u{1F30A}"
    }, 201);
  } catch (error3) {
    console.error("Error creating bottle:", error3);
    return jsonResponse({
...
__name(handleCreateBottle, "handleCreateBottle");
async function handleGetBottle(bottleId, env2) {
  if (!bottleId) {
    return jsonResponse({ error: "Bottle ID is required" }, 400);
  }
  try {
    const key = `bottle:${bottleId}`;
    const bottleData = await env2.BOTTLES_KV.get(key);
    if (!bottleData) {
    ...
    console.error("Error fetching bottle:", error3);
    return jsonResponse({ error: "Failed to fetch bottle" }, 500);
  }
}
__name(handleGetBottle, "handleGetBottle");
async function handleSearchBottles(request, env2) {
  const url = new URL(request.url);
  ...
  try {
    const keys = await env2.BOTTLES_KV.list({
      prefix: "bottle:",
      limit: 1e3
      // 限制搜索范围
    });
    ...
    return jsonResponse({
      ...
      message: limitedResults.length > 0 ? `\u{1F30A} \u6D77\u98CE\u5E26\u6765\u4E86 ${limitedResults.length} \u4E2A\u6F02\u6D41\u74F6` : "\u{1F30A} \u6D77\u9762\u5F88\u5E73\u9759\uFF0C\u6CA1\u6709\u627E\u5230\u76F8\u5173\u7684\u6F02\u6D41\u74F6"
    });
  } catch (error3) {
    console.error("Error searching bottles:", error3);
    return jsonResponse({ error: "Search failed" }, 500);
  }
}
__name(handleSearchBottles, "handleSearchBottles");
...
// .wrangler/tmp/bundle-62R8dA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-62R8dA/middleware-loader.entry.ts
